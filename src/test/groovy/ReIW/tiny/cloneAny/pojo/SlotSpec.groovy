...
class SlotSpec extends Specification {
	...
	def 'generic なクラスだけど型パラメタの子スロットは追加されない'() {
		...
		when:
		def Slot slot = Slot.getSlot(null, Type.getDescriptor(HashMap), null)
	...
	def 'object array class'() {
		...
		def Slot slot = Slot.getSlot(null, "[Ljava/lang/Integer;", null)
	...
	def 'primitive array class'() {
		when:
		def Slot slot = Slot.getSlot(null, Type.getDescriptor(int[][]), null)
